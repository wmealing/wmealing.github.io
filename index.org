...
* [[./concrete-callgraph.html][Concrete 005: Call Graphs and Dead Code - Aug 2026]]
:PROPERTIES:
:CUSTOM_ID: post-227f5e95
:END:

Fifth post, skipped straight past the compiler-internals posts I'd planned because dead-code elimination is
more useful to know about than IR records. Also found two real gaps while reading the actual walker code:
it doesn't look inside =if=/=try=/=receive=, and =apply/3= just vanishes with no warning, despite the project
notes promising one. Wrote it up honestly instead of glossing over it.
