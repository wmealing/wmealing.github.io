...
The first version of this demo did not close the curve correctly. It swept far enough around to draw the ring twice over, which reads exactly like drawing it once -- the second half just retraces the first. Getting the loop count and the total sweep to actually agree, not just look plausible, turned out to matter more than picking bigger numbers.
...
=test/js_exec_SUITE.erl= has the whole feature under test, run for real in Node, not just traced by hand. Each test case mocks a global the plain way a real page would get one: a bare =globalThis.Counter = class { ... }= assignment, injected before the compiled snippet runs. The test then calls =concrete_js:new/2=, =call/3=, =get/2=, and =set/3= against it, and checks the result. Two cases exist to prove argument unboxing works. One passes a Concrete list to a function that calls the native =.reduce= array method on it. One passes a map to a function that reads a plain object property off it. Both would throw at once if the argument still arrived boxed.

For the real thing, running in a real browser: start =rebar3 as example shell=, call =spirograph_demo:serve()=, and open =http://localhost:8773=.
